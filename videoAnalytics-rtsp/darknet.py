...
def performDetect(imagePath="data/giraffe.jpg", thresh= 0.25, configPath = "./cfg/yolov3.cfg", weightPath = "yolov3.weights", metaPath= "./cfg/coco.data", showImage= True, makeImageOnly = False, initOnly= False):
    """
...
def performBatchDetect(thresh= 0.25, configPath = "./cfg/yolov3.cfg", weightPath = "yolov3.weights", metaPath= "./cfg/coco.data", hier_thresh=.5, nms=.45, batch_size=3):
    ...
    img_samples = ['data/dog.jpg', 'data/eagle.jpg', 'data/giraffe.jpg']
    ...
    for b in range(batch_size):
        ...
        for i in range(num):
            ...
            if score > thresh:
                ...
                #detect_result.append(label)
               # detect_result.append((top,left))
                #detect_result.append((bottom,right))

                boxColor = (int(255 * (1 - (score ** 2))), int(255 * (score ** 2)), 0)
                cv2.rectangle(image_list[b], (left, top),
                          (right, bottom), boxColor, 2)
        
       # cv2.imwrite(fileNameList[b],image_list[b])
        
        
        #label_location_list.append(detect_result)
...
def performBatchDetect1(image=None,filePath='',batch_size=1, thresh= 0.25, configPath = "./cfg/yolov3.cfg", weightPath = "yolov3.weights", metaPath= "./cfg/coco.data", hier_thresh=.5, nms=.45):
    import cv2
    import numpy as np
    # NB! Image sizes should be the same
    # You can change the images, yet, be sure that they have the same width and height
    ...
    for b in range(batch_size):
        ...
        if(filePath!=None and filePath!=''):
          cv2.imwrite(filePath,image_list[b])
